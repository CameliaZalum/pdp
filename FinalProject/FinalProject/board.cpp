...
public:
    ...
    vector<MyState*> getNeighbors() {
        vector<MyState*> neighbors;
        if (emptyRow > 0) {
            Board* nbr = new Board(this, -1, 0);
            neighbors.push_back((MyState*)nbr);
        }
        if (emptyRow < size - 1) {
            Board* nbr = new Board(this, 1, 0);
            neighbors.push_back((MyState*)nbr);
    ...
    bool operator==(const MyState& other) {
        Board* b = (Board*)(&other);
    ...
    string toString() {
        ...
        */
        s += "emptyRow=" + to_string(emptyRow) + " ";
        s += "emptyCol=" + to_string(emptyCol) + "\n";
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
